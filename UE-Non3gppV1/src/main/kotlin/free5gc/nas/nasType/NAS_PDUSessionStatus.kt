...
class PDUSessionStatus {
    ...
    @JvmName("getLenPDUSessionStatus")
    fun getLen(): UByte {
        return len
    }

    @JvmName("setLenPDUSessionStatus")
    fun setLen(len: UByte) {
        this.len = len
        buffer = UByteArray(len.toInt())
    }

    @JvmName("getIeiPDUSessionStatus")
    fun getIei(): UByte {
        return iei
    }

    @JvmName("setIeiPDUSessionStatus")
...
*/
    ...
    fun setPSI0(pSI0: UByte) {
        buffer[0] = (buffer[0] and 254.toUByte()) or (pSI0 and 1u)
    }
...
*/
    ...
    fun setPSI8(pSI8: UByte) {
        buffer[1] = (buffer[1] and 254.toUByte()) or (pSI8 and 1u)
    }
    ...
    companion object {
        fun newPDUSessionStatus(iei: UByte): PDUSessionStatus {
            val pDUSessionStatus = PDUSessionStatus()
